...
module Splunk
  module Otel
    OUR_EXPORTERS_KEYS = ['jaeger-thrift-splunk']
    private_constant :OUR_EXPORTERS_KEYS

    module ExporterExtensions
      def get_effective_splunk_jaeger_endpoint
      ...
      def wrapped_exporters_from_env
        ...
        our_exporters = (exporters_keys & OUR_EXPORTERS_KEYS).map do |exporter_key|
          case exporter_key
          when 'jaeger-thrift-splunk'
            args = {
              endpoint: get_effective_splunk_jaeger_endpoint,
            }
            fetch_exporter_with_args('jaeger-thrift-splunk', 'OpenTelemetry::Exporter::Jaeger::CollectorExporter', **args)
          else
            nil
          end
        end.compact

        original_exporters + our_exporters
      end
